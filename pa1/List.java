...
public class List{
	private class Node{//the class node that will be consistently added to our list to add new data
		int item;//data contained in the list
		...
		Node(int x){//creates the node class with the items needed within it
	...
	int back(){//returns the item at the back of the list
	...
	int get(){//returns the item at the cursor
		if(numItems<=0){//if there is no items in the list throw an error message and exit
	...
	boolean equals(List L){//check if two lists are equal
		boolean eq=false;//start a boolean as false
		Node N=this.front;//start at the front of the first list
		Node M=L.front;//start at the front of the second list
		eq=((this.length())==(L.length()));//check that there lengths are the same
	...
	void prepend(int data){//insert to the front of the list
	...
	void append(int data){//insert to the back of the list
	...
	void insertBefore(int data){//insert a node before the cursor
	...
	void insertAfter(int data){//insert a node after the cursor 
		if(numItems<=0){//if we dont have any items in the list throw an error message and exit
	...
	List copy(){//copies the list over to a new list that is then returned
		List A=new List();//make a new lsit
		Node N=this.front;//point a node at the front of the old list
		while(N!=null){//while the old list is not empty
			A.append(N.item);//add the item to the back of the new list
